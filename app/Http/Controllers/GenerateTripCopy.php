...
class GenerateTripCopy extends Controller
{
    public function __invoke(Trip $trip)
    {
        ...
        $trip->load('people', 'days', 'days.events', 'days.events.contacts', 'days.events.participants', 'days.events.documents', 'days.events.links', 'articles', 'documents', 'links', 'collaborators');
        ...
        foreach($trip->getRelation('days') as $days){
            ...
            foreach($day->getRelation('events') as $events){
                ...
                foreach($event->getRelation('links') as $elink){
                    $event->links()->attach($elink);
                }
            }
        }
        ...
        foreach($trip->getRelation('links') as $links){
            $link = $links->replicate();
            $link->push();
            $new->links()->save($link);
        }
